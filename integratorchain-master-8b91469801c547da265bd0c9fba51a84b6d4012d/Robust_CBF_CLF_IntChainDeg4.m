function Robust_CBF_CLF_IntChainDeg4
clear;clc;close all;
m=1;
degree=4;   % degree where u is applied
...
plots();
    ...
    function plots()
        ...
        H = (x1(:,1)-x0).^2 + (x1(:,2)-y0).^2 + (x1(:,3)-z0).^2 - r0^2;
        plot(t1,H);
    ...
    function u = exp_CLFCBF_controller(t,xd,x)
        ...
        LgV = dV_dx*G;
        gamma=1;
        b = -LfV + dV_dx*xd_dot  - LgV*u_ff - gamma*V; %(x-xd)'*Q*(x-xd);
        A = [LgV -1];
        [B,dB_dx] = fcn_expCBF_IntChainDeg4(x,[x0 y0 z0 r0]);
        LfB = dB_dx*F*x;
        LgB = dB_dx*G;
        A = [A; -LgB 0];
...
%         title('Control Input');
    ...
    function u = CLFCBF_controller(t,xd,x)
        ...
        A = [LgV -1];
        [B,dB_dx] = fcn_CBF_IntChainDeg4(x,[x0 y0 z0 r0]);
        LfB = dB_dx*F*x;
        LgB = dB_dx*G;
        A = [A; LgB 0];
        b = [b; -LfB - LgB*u_ff + (gamma/B)];
